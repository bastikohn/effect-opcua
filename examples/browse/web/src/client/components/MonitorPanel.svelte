<script lang="ts">
  ...
  import { compactValue, sparkline } from "../value-format.js";
  ...
  let { rows, samplingIntervalMs, isMonitoring, onSamplingIntervalChange, onStart, onStop, onRemove }: Props = $props();
</script>
  ...
  <PanelHeader title="Monitored Variables">
    {#snippet actions()}
      <input
        ...
        oninput={(event) => onSamplingIntervalChange(event.currentTarget.valueAsNumber)}
      ...
      {:else}
        <IconButton title="Start monitoring" onclick={onStart} disabled={rows.length === 0}>
          <Play size={15} />
        </IconButton>
      {/if}
    {/snippet}
    ...
    {#each rows as row (row.nodeId)}
      <div class="grid gap-2 p-3">
        <div class="flex items-center justify-between gap-2">
          ...
          <IconButton class="shrink-0 text-stone-400" title="Remove" onclick={() => onRemove(row.nodeId)}>
            <Square size={13} />
          </IconButton>
        </div>
